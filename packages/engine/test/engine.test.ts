...
void test("marks evidence as expired when TTL is exceeded", () => {
      ...
      evidence_documents: [
        {
          ...
          source_url: "https://example.com/lvd-doc"
        }
      ]
    }
  };
  ...
  assert.equal(result.valid, false);
  assert.equal(result.result?.evaluations[0]?.status, "expired");
  assert.equal(result.result?.evaluations[0]?.blocking, true);
  assert.ok(result.result?.evaluations[0]?.auditTrail);
  assert.equal(result.result?.evaluations[0]?.auditTrail?.length, 1);
});

void test("warns when re-verification is due within warning period", () => {
  const reVerificationDueDate = new Date();
  reVerificationDueDate.setDate(reVerificationDueDate.getDate() - 1005);

  const reVerificationDueListing = {
    ...baseListing,
    product: {
      ...baseListing.product,
      evidence_documents: [
        {
          ...
          document_name: "LVD Declaration",
          status: "present",
          last_verified_at: reVerificationDueDate.toISOString().split('T')[0],
          source_url: "https://example.com/lvd-doc"
        }
      ]
    }
  ...
  assert.equal(result.valid, true);
  assert.equal(result.result?.evaluations[0]?.status, "reVerificationDue");
  assert.equal(result.result?.evaluations[0]?.blocking, false);
  assert.ok(result.result?.evaluations[0]?.reVerificationDueDate);
  assert.ok(result.result?.evaluations[0]?.auditTrail);
});

void test("handles conflicted evidence correctly", () => {
  const conflictedListing = {
    ...baseListing,
    product: {
      ...baseListing.product,
      evidence_documents: [
  ...
  assert.equal(result.valid, false);
  assert.equal(result.result?.evaluations[0]?.status, "conflicted");
  assert.equal(result.result?.evaluations[0]?.blocking, true);
  assert.ok(result.result?.evaluations[0]?.auditTrail);
  assert.equal(result.result?.evaluations[0]?.auditTrail?.length, 1);
});

void test("provides complete audit trail for evidence with multiple verifications", () => {
  const thirtyDaysAgo = new Date();
  thirtyDaysAgo.setDate(thirtyDaysAgo.getDate() - 30);

  const multiVerificationListing = {
    ...baseListing,
    product: {
      ...baseListing.product,
      evidence_documents: [
        {
          ...
          document_name: "LVD Declaration",
          status: "present",
          last_verified_at: thirtyDaysAgo.toISOString().split('T')[0],
          source_url: "https://example.com/lvd-doc"
        }
      ]
    }
  ...
  assert.equal(result.valid, true);
  assert.equal(result.result?.evaluations[0]?.status, "present");
  assert.ok(result.result?.evaluations[0]?.auditTrail);
  
  const auditTrail = result.result?.evaluations[0]?.auditTrail;
  assert.equal(auditTrail?.length, 1);
  assert.equal(auditTrail?.[0]?.decision, "verified");
  assert.equal(auditTrail?.[0]?.reason, "migrated_from_legacy");
  assert.equal(auditTrail?.[0]?.verifier, "system");
});
